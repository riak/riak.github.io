...
is a trivial process. In an [eventually consistent](../../learn/concepts/eventual-consistency.md), [clustered][concept clusters] system like Riak, however,
...
* A client attempts to read the object, Riak senses that there are
  divergent replicas and initiates a repair process (either [read repair](../../learn/concepts/active-anti-entropy.md#read-repair-vs-active-anti-entropy) or [active anti-entropy](../../learn/concepts/active-anti-entropy.md),
...
1. A tombstone object (`<<>>`) is written to N [vnodes][glossary vnode], with N
   defined by [`n_val`](../../developing/app-guide/replication-properties.md#n-value-and-replication)
...
If step 3 in the [process explained above](#tombstones) is reached, the `delete_mode` setting in your [configuration files](../../configuring/reference.md#advanced-configuration) will determine what happens next. This
...
when running [MapReduce jobs](../../developing/usage/mapreduce.md), but we do not recommend
this in general.

Setting `delete_mode` to a longer time duration than the default can be
useful in certain edge cases involving [Multi-Datacenter Replication](../cluster-operations/v3-multi-datacenter.md), e.g. when
